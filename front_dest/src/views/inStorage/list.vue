  ...
  <div>
    ...
    <a-table :columns="columns" bordered :loading="loading.list" :data-source="data" size="middle" :pagination='{
      ...
      <template slot="operation" slot-scope="text,record">
        ...
        <a-button type="link" @click='deleteInStorageList(record.id)'  v-if='$store.getters.userInfo.power == 1'>删除</a-button>
      </template>
    </a-table>
    <a-drawer
...
export default{
  ...
  methods:{
    ...
    async getList(){
      ...
      let res = await getInStorageList(params)
      console.log(res);
      this.data = res.data
    ...
    async submit(){
      ...
      console.log(this.form)
      let res = await addAndUpdateInStorage(params)
      console.log(res)
      if (res.data){
    ...
    async deleteInStorageList(id){
      const params = new URLSearchParams()
      params.append("id",id)
      let res = await deleteInStorage(params)
      if (!res.status){
